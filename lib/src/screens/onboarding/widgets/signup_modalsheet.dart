...
class _SignupModalSheetState extends State<SignupModalSheet> {
  ...
  void trySubmit(String email, String password, WidgetRef ref) {
    if (_isLogin) {
      try {
      ...
      try {
        ref.read(loginControllerProvider.notifier).signUp(email, password);
      } catch (error) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer(
      builder: (context, WidgetRef ref, child) => Wrap(children: <Widget>[
        Container(
          height: 550.h,
                          ...
                          LongButton(
                            ...
                            onPressed: () {
                              if (_formKey.currentState!.validate()) {
                                ...
                                trySubmit(_emailController.text,
